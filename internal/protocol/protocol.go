...
func serializeControlPacket(ctrlPktType uint8) uint8 {
	...
	assert(ctrlPktType <= 0x0F, "invalid ctrlPktType")
	assert(ctrlPktType != 0x00 && ctrlPktType != 0x0F, "invalid ctrlPktType, reserved")
	switch ctrlPktType {
	case pubrel, subscribe, unsubscribe:
		flags = 0x02
	default:
...
func serializeControlPacketPublish(qos uint8, setDup, setRetain bool) uint8 {
	...
	// assert qos is valid
	assert(qos == 0x00 || qos == 0x02 || qos == 0x04, "is invalid qos")
	var flags uint8 = qos
	if setDup {
...
/*
On server side when deserializing first byte
1. get ctrl packet type, if is reserved, error out
2. if is publish type, make sure the QOS level used is
		not reserved else error out
3. if is other type, flags should be the required reserved
